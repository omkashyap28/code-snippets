...
export default function SnippetCard({ snippet }: { snippet: Snippet }) {
  const { user } = useUser();
  const deleteSnippet = useMutation(api.snippets.deleteSnippets);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.error("Deleting snipppet unsucessfull");
    } finally {
      setIsDeleting(false);
    }
  };
            ...
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center gap-3">
                <div className="relative">
                  <div className="relative p-2 rounded-lg bg-linear-to-br from-blue-500/10 to-purple-500/20">
                    <Image
                ...
                <div className="space-y-1">
                  <span className="px-2 py-0.5 bg-blue-500/10 text-blue-400 rounded-sm text-xs font-medium">
                    {snippet.language}
                  </span>
              ...
              <div
                className="absolute top-5 right-5 z-10 flex gap-2 items-center"
                ...
                {user?.id === snippet.userId && (
                  <div className="z-10" onClick={(e) => e.preventDefault()}>
                    <button
                      ...
                      disabled={isDeleting}
                      className={`group flex items-center gap-1.5 py-1.5 px-2 rounded-lg transition-all duration-200
                                  ${
                                    isDeleting
